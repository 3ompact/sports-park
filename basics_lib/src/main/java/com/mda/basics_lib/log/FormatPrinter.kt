package com.mda.basics_lib.log
...
interface FormatPrinter {
     ...
     *
     * @param request 替换为自己相对的信息
     * @param bodyString 发送给服务器的请求体中的数据(已解析)
     */
    fun <T,S> printJsonRequest(request: T,bodyString: S)
     ...
     * @param request
     */
    fun <T> printFileRequest(request: T)
     ...
     * @param code         响应码
     * @param headers      请求头
     * @param contentType  服务器返回数据的数据类型(因为使用的okhttp3，一般情况下为MediaType)
     * @param bodyString   服务器返回的数据(已解析)
     * @param segments     域名后面的资源地址
     * @param message      响应信息
     * @param responseUrl  请求地址
     */
    fun <T> printJsonResponse(
        chainMs: Long,
