package com.mda.common_network.utils

import android.text.TextUtils
import com.mda.basics_lib.log.FormatPrinter
import com.mda.basics_lib.log.LogUtil
import com.mda.common_network.LogInterceptor.Companion.isJson
import com.mda.common_network.LogInterceptor.Companion.isXml
...
class NetworkFormatPrinter : FormatPrinter {
    /**
     ...
     * @param bodyString
     */
    override fun <Request, String> printJsonRequest(request: Request, bodyString: String) {
        val requestBody =
        ...
        logLines(
            tag,
            arrayOf(URL_TAG + (request as okhttp3.Request).url),
            false
        )
        logLines(
            tag,
            getRequest(request as okhttp3.Request),
            true
        )
        logLines(
            tag,
        ...
        printFileRequest(request as okhttp3.Request)
    }
     ...
     * @param request
     */
    override fun <T> printFileRequest(request: T) {
        val tag = getTag(true)
        LogUtil.debugInfo(tag, REQUEST_UP_LINE)
        logLines(
            tag,
     ...
     */
    override fun <MediaType> printJsonResponse(
        chainMs: Long,
        ...
        bodyString =
            when {
                isJson(contentType as okhttp3.MediaType) -> jsonFormat(bodyString!!)
                isXml(
                    contentType as okhttp3.MediaType
